...
class Dataset:
    ...
    def __init__(self) -> None:
        ...
        self.dish_vecs=np.load("F:/SHRINIVAS/KGP/BTP/Meal-Planning/Data/Processed/rec_vecs.npy")
        self.kdTree=KDTree(self.dish_vecs)

        self.ing_vector_model=KeyedVectors.load_word2vec_format("F:/SHRINIVAS/KGP/BTP/Meal-Planning/Data/Recipe1M/vocab.bin/vocab.bin", binary=True)
    ...
    def get_graph(self,file:str):
        return nx.read_edgelist("F:/SHRINIVAS/KGP/BTP/Meal-Planning/Data/Processed/graphs/"+file+".edgelist")
    ...
    def get_combi_dish(self,dish1:Dish,dish2:Dish)->float:
        if dish1.id not in self.combi_vocab or dish2.id not in self.combi_vocab:
            return 0
        return self.combi_model.similarity(dish1.id,dish2.id)
